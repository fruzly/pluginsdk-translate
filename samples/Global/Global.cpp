...
void GlobalDlgExecuteProc(void* pData)
{
	AFX_MANAGE_STATE(AfxGetStaticModuleState());
...
FS_BOOL PIInit(void)
{
	g_pLanguage = FRLanguageCreate(theApp.m_hInstance);
	FRLanguageChange(g_pLanguage, FRLanguageGetCurrentID());
...
FS_BOOL PIHandshake(FS_INT32 handshakeVersion, void *handshakeData)
{
	...
	pData->PIHDRegisterPlugin(pData, "Sample_Global", (FS_LPCWSTR)L"Global");
