#include "stdafx.h"
#include "bookmark_operation.h"
#include <atlbase.h>

Bookmark_Operation::Bookmark_Operation(HWND hNavPage)
{
	m_hNavPage = hNavPage;
	m_pDoc = nullptr;
	m_pDestDoc = nullptr;
...
void Bookmark_Operation::InitBookMark(FR_Document pDoc)
{
    m_pDoc = pDoc;;
...
FPD_Bookmark Bookmark_Operation::InitDocRootBookmark()
{
...
void Bookmark_Operation::DestroyBookmark(FPD_Bookmark hBookmark)
{
...
FPD_Bookmark Bookmark_Operation::AddChildBookmark(FPD_Bookmark hParent, FS_LPCSTR csTitle)
{
	...
	::SendMessage(m_hNavPage, SAMPLE_BOOKMARK_USERMSG_SHOWBOOKMARK, (WPARAM)pChildBookmark, NULL);

    return pChildBookmark;
}

//Add a bookmark of the same level after the entered bookmark object
FPD_Bookmark Bookmark_Operation::AddSiblingBookmark(FPD_Bookmark hPreBookmark, FS_LPCSTR csTitle)
{
    //Create an empty bookmark object
    ...
    //Send a signal to the interface to display a new bookmark
	::SendMessage(m_hNavPage, SAMPLE_BOOKMARK_USERMSG_SHOWBOOKMARK, (WPARAM)pSiblingBookmark, NULL);

    return pSiblingBookmark;
}

FPD_Bookmark Bookmark_Operation::FindBookmark(FPD_Bookmark hParent, FS_LPCSTR csTitle)
{
    USES_CONVERSION;
...
void Bookmark_Operation::GotoBookmarkDest(FPD_Bookmark hBookmark)
{
    if(m_pDoc == nullptr || m_pDestDoc == nullptr)
    {
        return;
...
void Bookmark_Operation::DeleteBookmark(FPD_Bookmark hDeleteBookmark)
{
    //Judging the validity of the entered bookmark
	...
	::SendMessage(m_hNavPage, SAMPLE_BOOKMARK_USERMSG_DELETEBOOKMARK, (WPARAM)hDeleteBookmark, NULL);
}

void Bookmark_Operation::ShowAllChildBookmark(FPD_Bookmark hParent)
{
    if(hParent != NULL)
    {
        //Judging the validity of the entered bookmark
    ...
    {
		//Send a signal to the interface to display a new bookmark
		::SendMessage(m_hNavPage, SAMPLE_BOOKMARK_USERMSG_SHOWBOOKMARK, (WPARAM)pChild, NULL);
        ...
        while(bGet)
        {
            ...
            if(bGet)
            {
                pChild = pNextChild;
				//Send a signal to the interface to display a new bookmark
				::SendMessage(m_hNavPage, SAMPLE_BOOKMARK_USERMSG_SHOWBOOKMARK, (WPARAM)pNextChild, NULL);
                //Recursive call to show children
...
void Bookmark_Operation::DeleteBookmarkAndChild(FPD_Bookmark hParent)
{
    //Judging the validity of the entered bookmark
    if(FALSE == FPDBookmarkIsVaild(hParent))
    {
...
FPD_Object Bookmark_Operation::AddNewBookmarkObject(FPD_Object hParentDic, FS_LPCSTR csTitle)
{
...
void Bookmark_Operation::AddParentBookmarkCount(FPD_Object hParentDic)
{
...
void Bookmark_Operation::DeleteParentBookmarkCount(FPD_Object hParentDic)
{
    //Get the dictionary information of the number of children of the input object
    //The plus or minus of the number of children indicates whether the bookmark is in the expanded state
    int count = FPDDictionaryGetInteger(hParentDic, "Count");
